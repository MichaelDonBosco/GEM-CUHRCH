...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const navItems = [
    ...
    { id: 'live', label: 'LIVE SANCTUARY', icon: Tv, badge: isLiveNow ? 'LIVE' : 'BROADCAST' },
  ...
  return (
    <header className="sticky top-0 z-50 shadow-xl shadow-black/25 w-full">
      {/* Refined Dignified Top Announcement & Quick-Info Bar */}
      <div className="bg-[#011712] border-b border-[#065f46]/50 text-slate-200 text-xs py-1.5 px-4 overflow-hidden">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          ...
          {/* Right: Worship Timings & Language Badge */}
      ...
      {/* Main Sanctuary Navigation Bar */}
      <div className="bg-[#043e32]/95 backdrop-blur-md border-b border-[#065f46]/40 text-white w-full">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
            ...
            {/* Desktop Navigation Links - UPPERCASE & Refined Alignment */}
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-2 py-1 text-[11px] font-bold font-cinzel uppercase tracking-wider rounded-lg transition-all duration-200 cursor-pointer flex items-center gap-1 whitespace-nowrap ${
                      isActive
                        ? 'text-[#fbbf24] bg-[#011712] border border-[#fbbf24]/60 shadow-sm'
            ...
            <nav className="hidden lg:flex xl:hidden items-center gap-1">
              {navItems.slice(0, 6).map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => handleNavClick(item.id)}
                    className={`relative px-2 py-1 text-[10px] font-bold font-cinzel uppercase tracking-wider rounded-md transition-all cursor-pointer whitespace-nowrap ${
                      isActive
                        ? 'text-[#fbbf24] bg-[#011712] border border-[#fbbf24]/60'
                        : 'text-emerald-100 hover:text-white hover:bg-white/10'
                    }`}
                  >
                    <span>{item.label}</span>
              ...
              <button
                ...
                className="px-2 py-1 text-[10px] font-bold font-cinzel uppercase tracking-wider text-[#fbbf24] bg-white/10 rounded-md cursor-pointer hover:bg-white/20"
              >
                MORE ▾
              </button>
            </nav>
            ...
            <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase font-cinzel tracking-wider bg-white/10 hover:bg-white/20 text-white border border-white/20 hover:border-[#fbbf24]/60 transition-all cursor-pointer shadow-sm"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold uppercase font-cinzel tracking-wider bg-gradient-to-r from-[#fbbf24] to-[#f59e0b] hover:from-[#f59e0b] hover:to-[#d97706] text-[#043e32] shadow-md shadow-black/30 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
              >
            ...
            <div className="lg:hidden flex items-center gap-1.5">
              <button
                ...
                onClick={() => handleNavClick('giving')}
                className="sm:hidden px-2.5 py-1 rounded-full text-[10px] font-bold uppercase font-cinzel tracking-wider bg-[#fbbf24] text-[#043e32]"
              >
                GIVE
              </button>
              <button
